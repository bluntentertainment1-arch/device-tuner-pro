...
struct BatterySaverModeView: View {
    ...
    private var mainToggleCard: some View {
        VStack(spacing: 16) {
            ...
            if viewModel.isBatterySaverActive {
                Text("Battery monitoring mode is tracking background activity and providing usage insights. Mode will automatically disable after 1 hour.")
                    .font(.system(size: 13, weight: .regular))
                    .foregroundColor(themeManager.secondaryText)
                    .frame(maxWidth: .infinity, alignment: .leading)
            }
    ...
    private var advancedBatteryInfoCard: some View {
        VStack(spacing: 16) {
            HStack {
